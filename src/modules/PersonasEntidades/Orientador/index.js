...
import OrientadorCreador from './OrientadorCreador';
import {
  onGetColeccion,
  onDelete,
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear Asesor' onClick={onOpenAddOrientador}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddOrientador: PropTypes.func.isRequired,
...
const Orientador = (props) => {
  ...
  const onOpenEditOrientador = (id) => {
    setOrientadorSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewOrientador = (id) => {
    setOrientadorSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteOrientador = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar El Asesor?',
  ...
  const onOpenAddOrientador = () => {
    setOrientadorSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      <Paper className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddOrientador={onOpenAddOrientador}
                        ...
                        <TableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditOrientador(row.id)}
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewOrientador(row.id)}
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            <Tooltip
                              ...
                              <DeleteIcon
                                onClick={() => onDeleteOrientador(row.id)}
      ...
      {showForm ? (
        <OrientadorCreador
          showForm={showForm}
          Orientador={OrientadorSeleccionado}
          accion={accion}
