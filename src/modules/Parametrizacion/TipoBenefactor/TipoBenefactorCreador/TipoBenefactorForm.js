...
const TipoBenefactorForm = (props) => {
  const {handleOnClose, accion, initialValues, titulo} = props;

  const [disabled, setDisabled] = useState(false);
  useEffect(() => {
    if (accion === 'ver' || initialValues.tipBenEstado === '0') {
      setDisabled(true);
    }
  }, [initialValues.tipBenEstado, accion]);
      ...
      <Scrollbar style={{maxHeight: 600}}>
        <Box py={5} px={{xs: 5, lg: 8, xl: 10}}>
          ...
          <Box px={{md: 5, lg: 8, xl: 10}} style={{display: 'flex', flexDirection: 'column'}}>
            <MyTextField className={classes.myTextField} label='Nombre*' name='tipBenDescripcion' disabled={disabled}/>
            <MyRadioField label='Estado' name='tipBenEstado' required /*disabled={disabled}*/ options={options}/>
          </Box>
        </Box>
      </Scrollbar>
...
export default TipoBenefactorForm;
