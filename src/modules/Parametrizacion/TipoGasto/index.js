...
import TipoGastoCreador from './TipoGastoCreador';
import {
  onGetColeccion,
  onDelete,
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear Tipo gasto' onClick={onOpenAddTipoGasto}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddTipoGasto: PropTypes.func.isRequired,
...
const TiposGasto = (props) => {
  ...
  const [tipoGastoSeleccionado, setTipoGastoSeleccionado] = useState(0);
  ...
  const onOpenEditTipoGasto = (id) => {
    setTipoGastoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewTipoGasto = (id) => {
    setTipoGastoSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteTipoGasto = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿ Seguro que desea eliminar el Tipo de gasto ?',
  ...
  const onOpenAddTipoGasto = () => {
    setTipoGastoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
      ...
      <Paper className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddTipoGasto={onOpenAddTipoGasto}
                        ...
                        <TableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditTipoGasto(row.id)}
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewTipoGasto(row.id)}
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            <Tooltip
                              ...
                              <DeleteIcon
                                onClick={() => onDeleteTipoGasto(row.id)}
