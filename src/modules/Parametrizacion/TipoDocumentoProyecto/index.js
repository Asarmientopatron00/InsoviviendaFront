...
import TipoDocumentoProyectoCreador from './TipoDocumentoProyectoCreador';
import {
  onGetColeccion,
  onDelete,
...
const EnhancedTableToolbar = (props) => {
  ...
  const {
    numSelected,
    titulo,
    onOpenAddTipoDocumentoProyecto,
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear Tipo documento proyecto' onClick={onOpenAddTipoDocumentoProyecto}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddTipoDocumentoProyecto: PropTypes.func.isRequired,
...
const TiposDocumentoProyecto = (props) => {
  ...
  const [tipoDocumentoProyectoSeleccionado, setTipoDocumentoProyectoSeleccionado] = useState(0);
  ...
  const onOpenEditTipoDocumentoProyecto = (id) => {
    setTipoDocumentoProyectoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewTipoDocumentoProyecto = (id) => {
    setTipoDocumentoProyectoSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteTipoDocumentoProyecto = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿ Seguro que desea eliminar el Tipo documento proyecto ?',
  ...
  const onOpenAddTipoDocumentoProyecto = () => {
    setTipoDocumentoProyectoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      <Paper className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddTipoDocumentoProyecto={onOpenAddTipoDocumentoProyecto}
                        ...
                        <TableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditTipoDocumentoProyecto(row.id)}
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewTipoDocumentoProyecto(row.id)}
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            <Tooltip
                              ...
                              <DeleteIcon
                                onClick={() => onDeleteTipoDocumentoProyecto(row.id)}
      ...
      {showForm ? (
        <TipoDocumentoProyectoCreador
          showForm={showForm}
          tipoDocumentoProyecto={tipoDocumentoProyectoSeleccionado}
          accion={accion}
...
export default TiposDocumentoProyecto;
