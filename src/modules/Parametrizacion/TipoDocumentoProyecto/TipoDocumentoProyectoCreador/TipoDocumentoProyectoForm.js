...
const options_requerido = [ {value: '1', label: 'Si'},
                            {value: '0', label: 'No'}, ];

const options = [ {value: '1', label: 'Activo'},
                  {value: '0', label: 'Inactivo'}, ];

const TipoDocumentoProyectoForm = (props) => {
  const {handleOnClose, accion, initialValues, titulo} = props;

  const [disabled, setDisabled] = useState(false);
  useEffect(() => {
    if (accion === 'ver' || initialValues.tiDoPrEstado === '0') {
      setDisabled(true);
    }
  }, [initialValues.tiDoPrEstado, accion]);
...
export default TipoDocumentoProyectoForm;
