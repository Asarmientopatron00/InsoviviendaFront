...
import FormaPagoCreador from './FormaPagoCreador';
import {
  onGetColeccion,
  onDelete,
} from '../../../redux/actions/FormaPagoAction';
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear Forma de pago' onClick={onOpenAddFormaPago}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddFormaPago: PropTypes.func.isRequired,
...
const TiposIdentificacion = (props) => {
  ...
  const [formaPagoSeleccionado, setFormaPagoSeleccionado] = useState(0);
  ...
  const onOpenEditFormaPago = (id) => {
    setFormaPagoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewFormaPago = (id) => {
    setFormaPagoSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteFormaPago = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿ Seguro que desea eliminar la Forma de pago ?',
      ...
      <Paper className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddFormaPago={onOpenAddFormaPago}
                        ...
                        <TableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditFormaPago(row.id)}
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewFormaPago(row.id)}
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            <Tooltip
                              ...
                              <DeleteIcon
                                onClick={() => onDeleteFormaPago(row.id)}
