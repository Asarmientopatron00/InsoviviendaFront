...
const FormaPagoForm = (props) => {
  const {handleOnClose, accion, initialValues, titulo} = props;

  const [disabled, setDisabled] = useState(false);
  useEffect(() => {
    if (accion === 'ver' || initialValues.forPagEstado === '0') {
      setDisabled(true);
    }
  }, [initialValues.forPagEstado, accion]);
      ...
      <Scrollbar style={{maxHeight: 600}}>
        <Box py={5} px={{xs: 5, lg: 8, xl: 10}}>
          ...
          <Box px={{md: 5, lg: 8, xl: 10}} style={{display: 'flex', flexDirection: 'column'}}>
            <MyTextField className={classes.myTextField} label='Nombre*' name='forPagDescripcion' disabled={disabled}/>
            <MyRadioField label='Estado' name='forPagEstado' required /*disabled={disabled}*/ options={options}/>
          </Box>
        </Box>
      </Scrollbar>
