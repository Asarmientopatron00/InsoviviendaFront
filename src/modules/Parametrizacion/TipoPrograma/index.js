...
import TipoProgramaCreador from './TipoProgramaCreador';
import {
  onGetColeccion,
  onDelete,
} from '../../../redux/actions/TipoProgramaAction';
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear Tipo de Programa' onClick={onOpenAddTipoPrograma}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddTipoPrograma: PropTypes.func.isRequired,
...
const TiposPrograma = (props) => {
  ...
  const [tipoProgramaSeleccionado, setTipoProgramaSeleccionado] = useState(0);
  ...
  const onOpenEditTipoPrograma = (id) => {
    setTipoProgramaSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewTipoPrograma = (id) => {
    setTipoProgramaSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteTipoPrograma = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar El Tipo de Programa?',
  ...
  const onOpenAddTipoPrograma = () => {
    setTipoProgramaSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      <Paper className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddTipoPrograma={onOpenAddTipoPrograma}
                        ...
                        <TableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditTipoPrograma(row.id)}
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewTipoPrograma(row.id)}
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            <Tooltip
                              ...
                              <DeleteIcon
                                onClick={() => onDeleteTipoPrograma(row.id)}
      ...
      {showForm ? (
        <TipoProgramaCreador
          showForm={showForm}
          tipoPrograma={tipoProgramaSeleccionado}
          accion={accion}
