...
import EpsCreador from './EpsCreador';
import {
  onGetColeccion,
  onDelete,
} from '../../../redux/actions/EpsAction';
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear EPS' onClick={onOpenAddEPs}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddEPs: PropTypes.func.isRequired,
...
const Eps = (props) => {
  ...
  const [EPsSeleccionado, setEPsSeleccionado] = useState(0);
  ...
  const onOpenEditEPs = (id) => {
    setEPsSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewEPs = (id) => {
    setEPsSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteEPs = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar El EPS?',
