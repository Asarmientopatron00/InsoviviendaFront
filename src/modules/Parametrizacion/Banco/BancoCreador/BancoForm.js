...
const BancoForm = (props) => {
  ...
  useEffect(() => {
    if (accion === 'ver' || initialValues.bancosEstado === '0') {
      setDisabled(true);
    }
  }, [initialValues.bancosEstado, accion]);
      ...
      <Scrollbar style={{maxHeight: 600}}>
        <Box py={5} px={{xs: 5, lg: 8, xl: 10}}>
          ...
          <Box px={{md: 5, lg: 8, xl: 10}} style={{display: 'flex', flexDirection: 'column'}}>
            <MyTextField className={classes.myTextField} label='Nombre*' name='bancosDescripcion' disabled={disabled}/>
            <MyRadioField label='Estado' name='bancosEstado' required /*disabled={disabled}*/ options={options}/>
          </Box>
        </Box>
      </Scrollbar>
