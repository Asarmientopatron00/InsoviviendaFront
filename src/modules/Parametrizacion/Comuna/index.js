...
import ComunaCreador from './ComunaCreador';
import {
  onGetColeccion,
  onDelete,
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear Comuna' onClick={onOpenAddComuna}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddComuna: PropTypes.func.isRequired,
...
const Comunas = (props) => {
  ...
  const [comunaSeleccionado, setComunaSeleccionado] = useState(0);
  ...
  const onOpenEditComuna = (id) => {
    setComunaSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewComuna = (id) => {
    setComunaSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteComuna = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar La comuna?',
  ...
  const onOpenAddComuna = () => {
    setComunaSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
      ...
      <Paper className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddComuna={onOpenAddComuna}
                        ...
                        <TableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditComuna(row.id)}
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewComuna(row.id)}
