...
import OcupacionCreador from './OcupacionCreador';
import {
  onGetColeccion,
  onDelete,
} from '../../../redux/actions/OcupacionAction';
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear Ocupacion' onClick={onOpenAddOcupacion}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddEstadoCivil: PropTypes.func.isRequired,
...
const Ocupacion = (props) => {
  ...
  const [OcupacionSeleccionado, setOcupacionSeleccionado] = useState(0);
  ...
  const onOpenEditOcupacion = (id) => {
    setOcupacionSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewOcupacion = (id) => {
    setOcupacionSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteOcupacion = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar la Ocupación?',
  ...
  const onOpenAddOcupacion = () => {
    setOcupacionSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
      ...
      <Paper className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddOcupacion={onOpenAddOcupacion}
                        ...
                        <TableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditOcupacion(row.id)}
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewOcupacion(row.id)}
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            <Tooltip
                              ...
                              <DeleteIcon
                                onClick={() => onDeleteOcupacion(row.id)}
