...
export const onGetColeccion = (
  ...
  return (dispatch) => {
    ...
    jwtAxios
      .get('tipos-benefactor', { params: { page: page,
                                           limite: rowsPerPage,
                                           nombre: nombreAux,
...
export const onGetColeccionLigera = () => {
  ...
  return (dispatch) => {
    dispatch({ type: FETCH_START });
    jwtAxios
      .get('tipos-benefactor', { params: { ligera: true, }, })
      .then((data) => { if (data.status === 200) {
                           dispatch({ type: FETCH_SUCCESS });
...
export const onUpdate = (params, handleOnClose, updateColeccion) => {
  return (dispatch) => {
    ...
    jwtAxios
      ...
      .then((data) => { if (data.status === 200) {
                           dispatch({ type: FETCH_SUCCESS });
                           dispatch({ type: UPDATE_TIPO_BENEFACTOR, payload: data.data, });
                           updateColeccion();
                           handleOnClose();
  ...
  return (dispatch) => {
    ...
    jwtAxios
      ...
      .then((data) => { if (data.status === 201) {
                           dispatch({ type: FETCH_SUCCESS });
                           dispatch({ type: CREATE_TIPO_BENEFACTOR, payload: data.data, });
                           updateColeccion();
                           handleOnClose();
                           dispatch({ type: SHOW_MESSAGE, payload: [data.data.mensajes[0], data.data.mensajes[1]], });
                          } 
