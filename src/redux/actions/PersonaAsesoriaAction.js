      ...
      jwtAxios
         ...
         .then((data) => { 
            if (data.status === 200) {
               ...
               dispatch({ type: GET_COLECCION_PERSONA_ASESORIA, payload: data });
            } 
            else 
   ...
   return (dispatch) => {
      ...
      jwtAxios
         ...
         .then((data) => { 
            if (data.status === 200) {
               dispatch({ type: FETCH_SUCCESS });
               dispatch({ type: GET_COLECCION_LIGERA_PERSONA_ASESORIA, payload: data });
            } 
            else 
               dispatch({ type: FETCH_ERROR, payload: messages['message.somethingWentWrong'], });
         })
...
export const onShow = (id) => {
   ...
   return (dispatch) => {
      if (id !== 0) {
         dispatch({ type: FETCH_START });
         jwtAxios
            .get('personas-asesorias/' + id)
            .then((data) => { 
               if (data.status === 200) {
                  dispatch({ type: FETCH_SUCCESS });
                  dispatch({ type: SHOW_PERSONA_ASESORIA, payload: data.data });
               } 
               else 
                  dispatch({ type: FETCH_ERROR, payload: messages['message.somethingWentWrong'], });
            })
...
export const onUpdate = (params, handleOnClose, updateColeccion) => {
   return (dispatch) => {
      dispatch({ type: FETCH_START });
      jwtAxios
         .put('personas-asesorias/' + params.id, params)
         .then((data) => { 
            if (data.status === 200) {
               dispatch({ type: FETCH_SUCCESS });
               dispatch({ type: UPDATE_PERSONA_ASESORIA, payload: data.data, });
               updateColeccion();
               handleOnClose();
...
export const onDelete = (id) => {
   return (dispatch) => {
      dispatch({ type: FETCH_START });
      jwtAxios
         .delete('personas-asesorias/' + id)
         .then((data) => { 
            if (data.status === 200) {
               dispatch({ type: FETCH_SUCCESS });
               dispatch({ type: SHOW_MESSAGE, payload: [data.data.mensajes[0], data.data.mensajes[1]], });
               dispatch({ type: DELETE_PERSONA_ASESORIA, payload: data.data });
            } 
            else 
               dispatch({ type: FETCH_ERROR, payload: data.data.mensajes[0] });
         })
...
export const onCreate = (params, handleOnClose, updateColeccion) => {
   return (dispatch) => {
      dispatch({ type: FETCH_START });
      jwtAxios
         .post('personas-asesorias', params)
         .then((data) => { 
            if (data.status === 201) {
               dispatch({ type: FETCH_SUCCESS });
               dispatch({ type: CREATE_PERSONA_ASESORIA, payload: data.data, });
               updateColeccion();
               handleOnClose();
               dispatch({ type: SHOW_MESSAGE, payload: [data.data.mensajes[0], data.data.mensajes[1]], });
            } 
