...
export const onGetColeccion = (
  ...
  const solicitanteAux = solicitante??'';
    ...
    jwtAxios
      ...
      .then((data) => {
        if (data.status === 200) {
          ...
          dispatch({type: GET_COLECCION_DESEMBOLSO, payload: data});
        } else {
          dispatch({
            type: FETCH_ERROR,
    ...
    if (id !== 0) {
      ...
      jwtAxios
        ...
        .then((data) => {
          if (data.status === 200) {
            dispatch({type: FETCH_SUCCESS});
            dispatch({type: SHOW_DESEMBOLSO, payload: data.data});
          } else {
            dispatch({
              type: FETCH_ERROR,
              payload: messages['message.somethingWentWrong'],
...
export const onUpdate = (params, handleOnClose, updateColeccion) => {
  return (dispatch) => {
    dispatch({type: FETCH_START});
    jwtAxios
      .put('desembolsos/' + params.id, params)
      .then((data) => {
        if (data.status === 200) {
          dispatch({type: FETCH_SUCCESS});
          dispatch({
            type: UPDATE_DESEMBOLSO,
            payload: data.data,
          });
          updateColeccion();
          handleOnClose();
    ...
    jwtAxios
      ...
      .then((data) => {
        if (data.status === 200) {
          ...
          dispatch({type: DELETE_DESEMBOLSO, payload: data.data});
        } else {
