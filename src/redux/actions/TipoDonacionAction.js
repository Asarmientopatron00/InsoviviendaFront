...
export const onGetColeccion = (
  ...
  return (dispatch) => {
    ...
    jwtAxios
      .get('tipos-donacion', { params: { page: page,
                                         limite: rowsPerPage,
                                         nombre: nombreAux,
      ...
      .then((data) => { if (data.status === 200) {
                           ...
                           dispatch({ type: GET_COLECCION_TIPO_DONACION, payload: data });
                          } 
                        else 
...
export const onGetColeccionLigera = () => {
  ...
  return (dispatch) => {
    dispatch({ type: FETCH_START });
    jwtAxios
      .get('tipos-donacion', { params: { ligera: true, }, })
      .then((data) => { if (data.status === 200) {
                           dispatch({ type: FETCH_SUCCESS });
                           dispatch({ type: GET_COLECCION_LIGERA_TIPO_DONACION, payload: data });
                          } 
                        else 
                           dispatch({ type: FETCH_ERROR, payload: messages['message.somethingWentWrong'], });
                      })
...
export const onShow = (id) => {
  ...
  return (dispatch) => {
    if (id !== 0) {
      dispatch({ type: FETCH_START });
      jwtAxios
        .get('tipos-donacion/' + id)
        .then((data) => { if (data.status === 200) {
                             dispatch({ type: FETCH_SUCCESS });
                             dispatch({ type: SHOW_TIPO_DONACION, payload: data.data });
                            } 
                          else 
                             dispatch({ type: FETCH_ERROR, payload: messages['message.somethingWentWrong'], });
                        })
...
export const onUpdate = (params, handleOnClose, updateColeccion) => {
  return (dispatch) => {
    dispatch({ type: FETCH_START });
    jwtAxios
      .put('tipos-donacion/' + params.id, params)
      .then((data) => { if (data.status === 200) {
                           dispatch({ type: FETCH_SUCCESS });
                           dispatch({ type: UPDATE_TIPO_DONACION, payload: data.data, });
                           updateColeccion();
                           handleOnClose();
...
export const onDelete = (id) => {
  return (dispatch) => {
    dispatch({ type: FETCH_START });
    jwtAxios
      .delete('tipos-donacion/' + id)
      .then((data) => { if (data.status === 200) {
                           dispatch({ type: FETCH_SUCCESS });
                           dispatch({ type: SHOW_MESSAGE, payload: [data.data.mensajes[0], data.data.mensajes[1]], });
                           dispatch({ type: DELETE_TIPO_DONACION, payload: data.data });
                          } 
                        else 
                           dispatch({ type: FETCH_ERROR, payload: data.data.mensajes[0] });
                      })
...
export const onCreate = (params, handleOnClose, updateColeccion) => {
  return (dispatch) => {
    dispatch({ type: FETCH_START });
    jwtAxios
      .post('tipos-donacion', params)
      .then((data) => { if (data.status === 201) {
                           dispatch({ type: FETCH_SUCCESS });
                           dispatch({ type: CREATE_TIPO_DONACION, payload: data.data, });
                           updateColeccion();
                           handleOnClose();
                           dispatch({ type: SHOW_MESSAGE, payload: [data.data.mensajes[0], data.data.mensajes[1]], });
                          } 
