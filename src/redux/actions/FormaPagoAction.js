...
export const onGetColeccion = (
  ...
  return (dispatch) => {
    ...
    jwtAxios
      .get('formas-pago', { params: { page: page,
                                      limite: rowsPerPage,
                                      nombre: nombreAux,
      ...
      .then((data) => { if (data.status === 200) {
                           ...
                           dispatch({ type: GET_COLECCION_FORMA_PAGO, payload: data });
                          } 
                        else 
...
export const onGetColeccionLigera = () => {
  ...
  return (dispatch) => {
    dispatch({ type: FETCH_START });
    jwtAxios
      .get('formas-pago', { params: { ligera: true, }, })
      .then((data) => { if (data.status === 200) {
                           dispatch({ type: FETCH_SUCCESS });
                           dispatch({ type: GET_COLECCION_LIGERA_FORMA_PAGO, payload: data });
                          }
                        else 
                           dispatch({ type: FETCH_ERROR, payload: messages['message.somethingWentWrong'], });
                      })
  ...
  return (dispatch) => {
    if (id !== 0) {
      ...
      jwtAxios
        ...
        .then((data) => { if (data.status === 200) {
                             dispatch({ type: FETCH_SUCCESS });
                             dispatch({ type: SHOW_FORMA_PAGO, payload: data.data });
                            }
                          else 
                             dispatch({ type: FETCH_ERROR, payload: messages['message.somethingWentWrong'], });
                        })
...
export const onUpdate = (params, handleOnClose, updateColeccion) => {
  return (dispatch) => {
    dispatch({ type: FETCH_START });
    jwtAxios
      .put('formas-pago/' + params.id, params)
      .then((data) => { if (data.status === 200) {
                           dispatch({ type: FETCH_SUCCESS });
                           dispatch({ type: UPDATE_FORMA_PAGO, payload: data.data, });
                           updateColeccion();
                           handleOnClose();
  ...
  return (dispatch) => {
    ...
    jwtAxios
      ...
      .then((data) => { if (data.status === 200) {
                           dispatch({ type: FETCH_SUCCESS });
                           dispatch({ type: SHOW_MESSAGE, payload: [data.data.mensajes[0], data.data.mensajes[1]], });
                           dispatch({ type: DELETE_FORMA_PAGO, payload: data.data });
                          } 
                        else 
                           dispatch({ type: FETCH_ERROR, payload: data.data.mensajes[0] });
                      })
...
export const onCreate = (params, handleOnClose, updateColeccion) => {
  return (dispatch) => {
    dispatch({ type: FETCH_START });
    jwtAxios
      .post('formas-pago', params)
      .then((data) => { if (data.status === 201) {
                           dispatch({ type: FETCH_SUCCESS });
                           dispatch({ type: CREATE_FORMA_PAGO, payload: data.data, });
                           updateColeccion();
                           handleOnClose();
                           dispatch({ type: SHOW_MESSAGE, payload: [data.data.mensajes[0], data.data.mensajes[1]], });
                          } 
